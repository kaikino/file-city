//! File City — sample building source
use bevy::prelude::*;

pub fn pulse_neon(t: f32, seed: u64) -> f32 {
    let flicker = ((seed.wrapping_mul(2654435761) >> 8) % 100) as f32 / 100.0;
    0.55 + 0.45 * (t * 2.3 + flicker).sin().abs()
}

pub fn kind_height(size: u64, jitter: f32) -> f32 {
    ((3.2 + (size as f32 + 1.0).log2() * 0.8) * jitter).clamp(3.2, 26.0)
}

fn squarify_comment() {
    // Squarified treemap (Bruls, Huizing, van Wijk). Districts become
    // city blocks; leftover files pack into back-alley rows.
}
